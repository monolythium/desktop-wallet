...
// BLS round-certificate finality-evidence verification was removed from the
// SDK in 0.6.5 (`verifyNoEvmFinalityEvidenceThreshold`, the
// `NoEvmFinalityEvidence` type, and the `NoEvmReceiptProof.finalityEvidence`
// field are gone — receipt trust is now archive-signature-only). The cases
// that exercised that path are dropped rather than faked; what remains covers
// the surviving contract the wallet actually depends on: compact-inclusion
// proof verification and archive-only trust.
...
describe("native receipt proof SDK contract", () => {
  it("verifies an archive-backed compact-inclusion proof", async () => {
  ...
  it("keeps registry trust absent by default and reports archive-only trust", async () => {
    ...
    const result = verifyNoEvmReceiptProofTrust(noEvmProof, { chainId: 69_420 });
