...
//
// The public API stays SYNC (readCursor / writeCursor / clearCursor).
...
export type CursorScope = "activity" | "discovery";

export interface CursorEntry<T> {
...
function cursorKey(scope: CursorScope, holder: string): string {
  return `${STORAGE_KEY_PREFIX}${scope}.${holder.toLowerCase()}`;
}

// ─── In-memory mirror ──────────────────────────────────────────────

const memMirror = new Map<string, CursorEntry<unknown>>();
...
async function idbReadAll(): Promise<Map<string, CursorEntry<unknown>>> {
  const out = new Map<string, CursorEntry<unknown>>();
  let db: IDBDatabase;
  try {
    db = await openDb();
  } catch {
  ...
  return new Promise((resolve) => {
    ...
    req.onsuccess = () => {
      ...
      if (cursor) {
        ...
        const value = cursor.value as CursorEntry<unknown>;
        out.set(key, value);
        cursor.continue();
      } else {
        db.close();
...
function idbWrite(key: string, entry: CursorEntry<unknown>): void {
...
export function hydrateLogCursorStore(): Promise<void> {
    ...
    try {
      ...
      for (const k of keys) {
        ...
        try {
          ...
          const entry: CursorEntry<unknown> = {
...
export function readCursor<T>(
  ...
  return entry as CursorEntry<T>;
}

/** Write a fresh cursor entry. Sync write to the in-memory mirror;
 *  async fire-and-forget into IDB. */
export function writeCursor<T>(
  scope: CursorScope,
  holder: string,
  entry: CursorEntry<T>,
): void {
  const key = cursorKey(scope, holder);
  memMirror.set(key, entry as CursorEntry<unknown>);
  idbWrite(key, entry as CursorEntry<unknown>);
}

/** Remove a cursor (used by manual "Refresh" / force-rescan). */
export function clearCursor(scope: CursorScope, holder: string): void {
  const key = cursorKey(scope, holder);
  memMirror.delete(key);
  idbDelete(key);
}
...
export function _resetAllCursorsForTest(): void {
  clearMirror();
  idbClearAll();
...
export function computeScanWindow(args: {
  ...
  const cursor = readCursor<unknown>(args.scope, args.holder);
