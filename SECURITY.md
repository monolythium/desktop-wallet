...
- exfiltrate the encrypted vault from the OS keychain, the Argon2id-derived KEK, the in-memory unlocked seed, or a recovery phrase outside the explicit reveal flow,
- bypass the password / keychain authentication gate before signing a destructive action,
- bypass the Operations drawer's `preview → auth → executing → done` state machine for any send / sign / stake / trade flow,
- escape the OS sandbox (read another app's keychain entries or write outside the wallet's app-data directory),
...
- Issues in upstream dependencies (`@noble/*`, `@scure/*`, `keyring`, `ethers`, Tauri plugins) — please report those upstream and we'll pick up the fix.
