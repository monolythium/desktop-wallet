...
//! - `outbound_mcp` — exposes Stele's surface to external automation clients
//!   (desktop MCP clients) over a loopback MCP server.
//!
...
/// external automation clients. `None` until the user enables it in
/// `Settings → MCP`.
#[derive(Default, Clone)]
