...
//! (desktop MCP clients) as MCP tools over HTTP.
//!
//! User flow:
...
//!     the inbound sidecar uses — The client cannot book a service without
...
async fn require_approval(
    ...
    let summary = match tool {
        "request_booking" => format!(
            "Client is asking to open a booking for {} LYTH",
            args.get("proposed_price_lyth")
                .and_then(|v| v.as_str())
                .unwrap_or("?"),
        ),
        "accept_booking" => format!(
            "Client is asking to accept booking {}",
            args.get("booking_id").and_then(|v| v.as_str()).unwrap_or("?"),
        ),
        "release_booking" => format!(
            "Client is asking to release escrow on booking {}",
            args.get("booking_id").and_then(|v| v.as_str()).unwrap_or("?"),
        ),
        _ => format!("Client wants to invoke {}", tool),
    };
    let req = ApprovalRequest {
        ...
        wallet: None,
        source: Some(serde_json::json!({ "client": "outbound MCP client" })),
        expires_at: None,
    };
