...
- **Tool-assisted code without an honest commit author.** If you used MCP client / local tool / desktop MCP client / local tool to write code, sign the commit with YOUR identity, not the tool's. We rewrote the public history once to scrub stale author identities; please don't reintroduce them.
